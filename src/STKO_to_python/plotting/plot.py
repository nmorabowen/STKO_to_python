...
class Plot:
    ...
    def plot_nodal_results(
        ...
        if results_name_verticalAxis not in ['STEP', 'TIME']:
            ...
            vertical_results_df = self.dataset.nodes.get_nodal_results(
            ...
            # Aggregate results
            y_array = self._aggregate_results(vertical_results_df, direction_verticalAxis, values_operation_verticalAxis) * scaling_factor_verticalAxis
        elif results_name_verticalAxis == 'STEP':
            y_array = self.dataset.time.loc[model_stage].index.tolist()
        else:
            y_array = self.dataset.time.loc[model_stage]['TIME'].values
        
        if results_name_horizontalAxis not in ['STEP', 'TIME']:
            # Retrieve results for horizontal axes
            horizontal_results_df = self.dataset.nodes.get_nodal_results(
                model_stage=model_stage,
            ...
            x_array = self.dataset.time.loc[model_stage].index.tolist()
        else:
